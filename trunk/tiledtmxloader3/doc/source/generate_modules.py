...
def recurse_tree(path, excludes, opts):
    ...
    for root, subs, files in tree:
        ...
        # exclude private modules, but not __init__.py files
        # py_files = sorted([f for f in py_files if not os.path.splitext(f)[0].startswith('_') or os.path.splitext(f)[0].startswith('__')])
        py_files = sorted([f for f in py_files if os.path.splitext(f)[0].find("_path.py") < 0])
        if INIT in py_files:
